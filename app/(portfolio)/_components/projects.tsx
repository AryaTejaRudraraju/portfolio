...
const CardContent: FC<CardContentProps> = ({ children, className = '' }) => ( <div className={`px-5 md:px-6 pb-5 md:pb-6 grow ${className}`}>{children}</div> );
        ...
        <Card className="h-full hover:border-neutral-600/80 transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-teal-500/10">
          <CardHeader className="pb-3 md:pb-4">
            <div className="flex justify-between items-start gap-2">
              ...
              <Badge variant={statusBadgeVariant} className="whitespace-nowrap mt-0.5 shrink-0">
                {project.status}
              </Badge>
            </div>
          </CardHeader>
            ...
            <div className="flex justify-start items-center gap-2 mb-4 pt-1 h-[32px]"> {/* Fixed height for one row of sm buttons */}
              {project.githubLink && (
                <Button variant="outline" size="sm" asChild href={project.githubLink} className="flex items-center">
                  <span>
                    ...
                    <GithubIcon size={14} className="mr-1.5 size-3.5" /> {/* Adjusted size */}
                    Code
                  </span>
                </Button>
              )}
              {project.liveLink && (
                <Button variant="outline" size="sm" asChild href={project.liveLink} className="flex items-center">
                  <span>
                    <ArrowRightIcon className="inline-block size-3.5 mr-1.5" />
                    Demo
                  </span>
                </Button>
              )}
        ...
        >
          {hasMoreProjects && (
            <Button variant="outline" size="lg" asChild href="https://github.com/your-username">
              <span> Explore More on GitHub <ArrowRightIcon className="inline-block size-4 ml-2" /> </span>
            </Button>
          )}
